...
TEST(OLEDTestGroup, PrintCharPtrTest) 
{
    // Setup
    OLED display(8, 2);
    display.clear();
...
TEST(OLEDTestGroup, PolymorphismTest) 
{
    // Setup
    Display* display = new OLED(40, 2);  // OLED "is a" Display (Upcast: inherently safe) 
    display->clear();
    
    // Exercise
    display->print("Hallo!");

    OLED* oled = dynamic_cast<OLED*>(display); // Explicit Downcast

    // Verify
    ASSERT_NE(nullptr, oled);
    EXPECT_STREQ("Hallo!", oled->buffer());
